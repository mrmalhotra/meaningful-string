...
function getRandom(object) {
  ...
  if (object && object.custom) referralString = object.custom;

  if (object && object.allCaps) referralString = upperCaseAlphabats;

  if (object && object.allSmall) referralString = smallCaseAlphabats;

  if (object && object.capsWithNumbers) referralString = upperAlphanumeric;

  if (object && object.smallWithNumbers) referralString = smallAlphanumeric;

  if (object && object.onlyNumbers) referralString = number;

  if (object && object.max) lengthShouldBeAtlast = parseInt(object.max);

  if (object && object.min) lengthShouldBeAtleast = parseInt(object.min);
  ...
  if (lengthShouldBeAtleast > lengthShouldBeAtlast) {
    ...
    console.log(
      "checking swaping value ",
      lengthShouldBeAtleast,
      " last ",
      lengthShouldBeAtlast
    );
  }

  if (object && object.max && object.min)
    lengthShouldBe =
      Math.floor(Math.random() * (object.max - object.min + 1)) + object.min;
  ...
  if (object && object.startWith) string = object.startWith + string;

  if (object && object.endWith) string = string + object.endWith;
...
const meaningful = (object) => {
  ...
  let chooseAdjective =
    adjective.adjective[
      Math.floor(Math.random() * (adjective.adjective.length - 1))
    ];

  if (object && object.numberUpto > 0) upto = parseInt(object.numberUpto);

  if (object && object.joinBy) joinString = object.joinBy;
...
const shortId = (object) => {
  ...
  if (object) object.fromShortId = true;
  ...
  if (
    object &&
    object.charLength &&
    parseInt(object.charLength) >= 3 &&
    parseInt(object.charLength) <= 8
  ) {
...
const uuidv4 = () => {
  // Define the UUID template with placeholders for each character
  return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function (c) {
    // Inside the replacement function, generate random hexadecimal digits
    var r = (Math.random() * 16) | 0; // Generate a random number between 0 and 15
    var v = c == "x" ? r : (r & 0x3) | 0x8; // For 'x', use the random digit; for 'y', use a specific pattern
    return v.toString(16); // Convert the generated number to a hexadecimal string
  });
};
